  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        sources:
          Array.isArray(initialData.sources) && initialData.sources.length > 0
            ? initialData.sources
            : [""],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `サーバーレスポンスの解析に失敗しました: ${response.status} ${response.statusText}`
        );
      }

      if (!response.ok) {
        const errorMessage =
          result.message || result.error || `HTTPエラー: ${response.status}`;
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <div style={styles.inputGroup}>
                ...
                <input
                  ...
                  onBlur={(e) =>
                    (e.target.style.borderColor = !formData.title.trim()
                      ? "#ef4444"
                      : "#d1d5db")
                  }
                />
                ...
                <div style={styles.dateRow}>
                  <div>
                    <input
                      ...
                      style={{
                        ...styles.input,
                        borderColor: !formData.date_start
                          ? "#ef4444"
                          : "#d1d5db",
                      }}
                      onFocus={(e) => (e.target.style.borderColor = "#3b82f6")}
                      onBlur={(e) =>
                        (e.target.style.borderColor = !formData.date_start
                          ? "#ef4444"
                          : "#d1d5db")
                      }
                    />
              ...
              <div style={styles.inputGroup}>
                ...
                <textarea
                  ...
                  onFocus={(e) => (e.target.style.borderColor = "#3b82f6")}
                  onBlur={(e) =>
                    (e.target.style.borderColor = !formData.description.trim()
                      ? "#ef4444"
                      : "#d1d5db")
                  }
                />
              ...
              <div style={styles.inputGroup}>
                ...
                {/* 自動抽出タグ */}
                {extractTagsFromDescription(formData.description).length >
                  0 && (
                  <div style={styles.autoTagsContainer}>
                    ...
                    {extractTagsFromDescription(formData.description).map(
                      (tag, index) => (
                        <button
                          key={index}
                          type="button"
              ...
              <div style={styles.inputGroup}>
                ...
                >
                  ...
                  <option value="GFDL-1.3">
                    GFDL-1.3（GNU自由文書ライセンス）
                  </option>
                </select>
                <div style={styles.hint}>
                  コンテンツの利用条件を選択してください
              ...
              <div style={styles.buttonContainer}>
                <button
                  ...
                  disabled={loading}
                  onMouseEnter={(e) =>
                    !loading && (e.target.style.backgroundColor = "#e5e7eb")
                  }
                  onMouseLeave={(e) =>
                    !loading && (e.target.style.backgroundColor = "#f3f4f6")
                  }
                >
                  キャンセル
                </button>
            ...
            <div style={styles.previewContent}>
              ...
              {formData.date_start && (
                ...
                >
                  ...
                  {formData.date_end &&
                    formData.date_end !== formData.date_start &&
                    ` - ${new Date(formData.date_end).toLocaleDateString(
                      "ja-JP"
                    )}`}
                </div>
              )}
