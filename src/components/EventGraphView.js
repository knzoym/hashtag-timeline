...
const EventGraphView = ({
      ...
      timelineColors: timelines
        .filter(
          (timeline) =>
            ...
            (timeline.temporaryEvents || []).some(
              (tlEvent) => tlEvent.id === event.id
            )
        )
  ...
  useEffect(() => {
    ...
    node
      ...
      .on("dblclick", (event, d) => {
        onEventDoubleClick &&
          onEventDoubleClick(events.find((e) => e.id === d.id));
      })
    ...
    <div style={styles.container} ref={containerRef}>
      ...
      <div style={styles.controlPanel}>
        ...
        <div style={styles.statsPanel}>
          <div style={styles.statItem}>📊 全イベント: {stats.totalEvents}</div>
          <div style={styles.statItem}>👁️ 表示中: {stats.visibleEvents}</div>
          <div style={styles.statItem}>📈 年表数: {stats.totalTimelines}</div>
