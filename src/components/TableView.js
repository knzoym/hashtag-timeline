...
const TableView = ({ 
  ...
  const sortedEvents = useMemo(() => {
    ...
    if (selectedTimeline === 'all') {
      ...
      // 特定の年表のイベント
      const timeline = timelines.find(t => t.id.toString() === selectedTimeline);
      eventsToShow = timeline ? timeline.events.map(event => ({
        ...event,
        timelineInfo: [{
          name: timeline.name,
          color: timeline.color
  ...
  }, [events, timelines, sortField, sortOrder, selectedTimeline, showTimelineEvents]);
  ...
  // インライン編集の保存
  const saveEdit = useCallback((eventId, field, newValue) => {
    ...
    if (field === 'title' || field === 'description') {
      const extractedTags = extractTagsFromDescription(updatedEvent.description || '');
  ...
  // タグの削除
  const removeTag = useCallback((eventId, tagToRemove) => {
    const event = sortedEvents.find(e => e.id === eventId);
    if (!event) return;
  ...
  // タグの追加
  const addTag = useCallback((eventId, newTag) => {
    const event = sortedEvents.find(e => e.id === eventId);
    if (!event || !newTag.trim() || event.tags.includes(newTag.trim())) return;
  ...
  const renderTimelineInfo = (timelineInfo) => {
    ...
    // 名前でユニークにする
    const uniqueInfos = timelineInfo.filter((info, index, array) => 
          ...
          return (
            <div 
              ...
              style={{ 
                display: 'grid',
                gridTemplateColumns: '1fr 100px 200px 2fr 2fr 100px',
                borderBottom: index < sortedEvents.length - 1 ? '1px solid #f3f4f6' : 'none',
            ...
            >
              ...
              <div style={{ padding: '12px', color: '#6b7280' }}>
                {editingCell?.eventId === event.id && editingCell?.field === 'startDate' ? (
                  ...
                  <div
                    onClick={() => startEdit(event.id, 'startDate', event.startDate)}
                    style={{
                      cursor: 'pointer',
                      padding: '4px',
                      borderRadius: '4px',
        ...
        {selectedTimeline !== 'all' && (
          <span> • フィルター: {
            selectedTimeline === 'main' 
              ? 'メインタイムライン' 
              : timelines.find(t => t.id.toString() === selectedTimeline)?.name
          }</span>
        )}
        {sortField && (
          <span> • ソート: {
            sortField === 'startDate' ? '日付' :
            sortField === 'title' ? 'タイトル' :
            sortField === 'tags' ? 'タグ数' : sortField
          }順{sortOrder === 'asc' ? '（昇順）' : '（降順）'}</span>
        )}
      </div>
    </div>
  );
